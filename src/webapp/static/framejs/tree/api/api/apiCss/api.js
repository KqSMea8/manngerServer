var apiContent = {
	...
	_init: function() {
		...
		var treenode_nodes =[
			...
			{id:111, pId:1, t:"name", name:"name", iconSkin:"core", showAPI:true},
			{id:112, pId:1, t:"nocheck", name:"nocheck", iconSkin:"check", showAPI:true},
			{id:113, pId:1, t:"open", name:"open", iconSkin:"core", showAPI:true},
			{id:113, pId:1, t:"target", name:"target", iconSkin:"core", showAPI:true},
			{id:113, pId:1, t:"url", name:"url", iconSkin:"core", showAPI:true},
		...
		var function_nodes =[
			{id:4, pId:0, t:"treeObj", name:"treeObj : {", open:true},
			...
			{id:431, pId:4, t:"getNodesByFilter", name:"getNodesByFilter (filter, isSingle, parentNode)", iconSkin:"core", showAPI:true},
	...
	overlayShow: function(target, isSameNode) {
		...
		ah =$("#RightPane").scrollTop() ,
		
		ifdisplay = $("#LeftPane").css('display')=='block'?true:false,
	    t = target.offset().top - 30-100+ah,
		//t = ah.top - 30-100,
		
		cMaxLeft = c.offset().left + c.outerWidth({margin:true}) - o.outerWidth({margin:true}) - 10,
		l = ifdisplay?Math.min(cMaxLeft, target.offset().left + target.width() + 40)-140:Math.min(cMaxLeft, target.offset().left + target.width() + 40),
		//l = Math.min(cMaxLeft, target.offset().left + target.width() + 40),
		arrowT = target.offset().top + 16- 80+ah,
		//arrowT = target.offset().top + 16,
		wMinTop = 100, footerHeight = 50, onlyFade = false,
		wHeight = w.height(), wScrollTop=ah, wMaxTop = wHeight + wScrollTop - footerHeight;
	...
	overlayAjax: function(treeId, node) {
		...
		$.ajax({
			type: "get",
			url: "./tree/api/api/" + lang + "/" + node.tt.replace("$.", "") + ".html",
			data: null,
			dataType: "text",
