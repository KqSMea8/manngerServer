...
import com.lmxf.post.tradepkg.partner.OrderApplyReq;
import com.lmxf.post.tradepkg.partner.OrderApplyResp;
 ...
 * 
 */
public class TradeOrderApplyService extends TradeProcess {
	private final static Log log = LogFactory.getLog(TradeOrderApplyService.class);
	private OrderApplyReq orderApplyReq;
	private OrderApplyResp orderApplyResp;
	...
	public String tradeProcess(String apply_xml) throws SystemException {
		...
		Map ret = null;
		try {
			ret = orderApplyReq.parseXml(apply_xml);
			if (ret.get("code") == null) {
				return errorCodeDao.getErrorRespJson(GConstent.Xml_Parse_Error);
			} else if (ret.get("code") != null && Integer.parseInt(ret.get("code").toString()) != 0) {
				log.error("orderApplyReq.parseXml is error!");
		...
		if (null != orderApplyList && orderApplyList.size() > 0) {
			...
			if (null != orderProductList && orderProductList.size() > 0) {
				for (OrderProduct orderProduct : orderProductList) {
					...
					List<SupplyVproduct> supplyVproductList = supplyVproductDao.findSupplyVproduct(supplyVproductP);
					...
					if (noOutTotalNum < orderProduct.getSaleNum()) {
						log.error("订单申请模块  站点编号:" + orderApply.getSiteId() + " 商品编号:" + orderProduct.getProductId() + " 在货道补货记录流水中没有足够的库存 库存数为:" + noOutTotalNum + " 无法分配出库货道.");
		...
		return orderApplyResp.CreateJson(GConstent.SUCCESS_CODE, GConstent.SUCCESS_MSG, 1, 1, orderApply);
	}

	public void setOrderApplyReq(OrderApplyReq orderApplyReq) {
		this.orderApplyReq = orderApplyReq;
	}

	public void setOrderApplyResp(OrderApplyResp orderApplyResp) {
