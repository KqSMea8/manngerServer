...
public class UnderInfoReq extends ReqHead {
	...
	public Map parseXml(String report_json) {
		...
		try {
			...
			productVUnder.setLoginId(JSONBody.getString("LoginId"));
			productVUnder.setCurState(JSONBody.getString("State"));
			productVUnder.setSiteId(JSONBody.getString("SiteId"));
			if (productVUnder.getLoginId() == null || "".equals(productVUnder.getLoginId())) {
		...
		r.put("code", 0);
		r.put("productVUnder", productVUnder);
		return r;
	}
