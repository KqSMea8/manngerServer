function showPages(name, currentpage) { //初始化属性
 this.name = name;      //对象名称
 this.page = 1;         //当前页数
 this.pageCount = currentpage;    //总页数
...
showPages.prototype.createHtml = function(mode){ //生成html代码
 ...
 switch (mode) {
  ...
  case 2 : //模式2 (前后缩略,页数,首页,前页,后页,尾页)
   strHtml += '<span class="count">分页: ' + this.page + ' / ' + this.pageCount + '</span>';
   strHtml += '<span class="number">';
   if (prevPage < 1) {
...
showPages.prototype.toPage = function(page){ //页面跳转
 ...
 document.getElementById("pages_currentPage").value = turnTo;
// alert(document.getElementById("pages_currentPage").value);
...
// alert("3="+this.page);
 ...
 document.write('<input type="hidden" name="pages_currentPage" id="pages_currentPage" value="">');
